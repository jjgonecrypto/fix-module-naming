...
module.exports = function () {
    ...
    getJSFiles().forEach(function (file) {
        ...
        //rename
        //fs.renameSync(file, path.join(path.basedir(file), newName));
    ...
    getJSFiles().forEach(function (file) {
        ...
        requires.forEach(function (requireNode) {
            ...
            if (newName !== path.basename(relPath) + '.js') {
                var newRequire = path.join(relDir, newName);
                if (relDir === '.') newRequire = '.' + path.sep + newRequire;
    ...
    //foreach files
        ...
        //if map exists, change require ending to new file
